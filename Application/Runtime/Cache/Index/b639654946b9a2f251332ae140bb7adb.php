		...
		<div class="order_status_bar">	
			...
			<div class="flowStep" style="width:576px;">	
				...
				<div class="text">					
					...
					<div class="active" style="width:120px;"><?php echo (Date("Y-m-d H:i:s",$buy_time)); ?></div>		
			...
			<div class="status_text">
					您的订单已经提交，请在
					<span class="red_f"><?php echo (Date("Y-m-d H:i:s",$to_pay_time)); ?></span>
					前完成支付，超时订单将自动关闭。
				</div>
		...
		<div class="order_goods">
			<table class="order_list">		
				...
				<tbody class="order_list">
					<?php if(is_array($goods_message)): $i = 0; $__LIST__ = $goods_message;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods): $mod = ($i % 2 );++$i;?><tr class="order_goods_list">		
						<td class="order_goods_goods">			
							<a target="_blank" href="<?php echo U('detail/index',array('gid'=>$goods['gid']));?>">
							<img class="pic" src="/shop/Public/<?php echo ($goods["big_picture"]); ?>"/>						
								<p><?php echo ($goods["first_name"]); ?></p>		
								<p class="prop">																	
									<span class="order_detail_prop">颜色：<?php echo ($goods["color"]); ?></span>	<br>											
									<span class="order_detail_prop">尺码：<?php echo ($goods["size"]); ?></span>						
								</p>			
							</a>			
						</td>			
						<td class=""><?php echo ($goods["sale_price"]); ?></td>	
						<td class=""><?php echo ($goods["nums"]); ?></td>			
						<td  rowspan="1"><?php echo ($goods["total"]); ?></td>		
						<td  rowspan="1"><?php echo ($goods["status"]); ?></td>			
					</tr>
					<tr class="order_goods_list">
						<td style="text-align: left;width: 300px">
							&nbsp;&nbsp;订单号：<?php echo ($goods["order_code"]); ?>
						</td>
						<td style="text-align: left;width: 300px">
							下单时间：<?php echo (Date("Y-m-d H:i:s",$goods["buy_time"])); ?>
						</td>
						<?php if($goods['pay_time'] > 0): ?><td style="text-align: left;width: 300px">
							支付时间：<?php echo (Date("Y-m-d H:i:s",$goods["pay_time"])); ?>
						</td><?php endif; ?>
						<?php if($goods['finish_time'] > 0): ?><td style="text-align: left;width: 300px">
							订单完成时间：<?php echo (Date("Y-m-d H:i:s",$goods["finish_time"])); ?>
						</td><?php endif; ?>
			...
			<ul>	
				...
				<li>		
					<p class="right_f">		
						<span class="last">		
							<span>运费：<em class="red_f f15_f"><?php echo ($yunfei); ?></em>元</span>		
							<span>实付金额：<strong class="red_f f18_f"><?php echo ($count_price); ?></strong>元</span>		
						</span>		
					</p>		
				</li>	
			</ul>
...
<!-- <script type="text/javascript" src="/shop/Public/index/js/json2.js"></script> -->
<script type="text/javascript">
	$("#pay_now").click(function(){
		var url="<?php echo U('order/payall');?>";
		var count="<?php echo ($i); ?>";
		var to_pay_time="<?php echo ($to_pay_time); ?>";
		// var json_str="";
	...
	$("#cancel_order").click(function(){
		var url="<?php echo U('order/cancelall');?>";
		var count="<?php echo ($i); ?>";
		$.ajax({
			url:url,
			type:"post",

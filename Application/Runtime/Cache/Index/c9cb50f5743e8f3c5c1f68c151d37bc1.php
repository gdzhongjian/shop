...
<script type="text/javascript" src="/shop/Public/index/layer/layer.js"></script>	
		...
		<form action="<?php echo U('order/addorder');?>" method="post" id="order_submit">
		<div class="container">	
			...
			<div class="order">		
				<table cellpadding="0" cellspacing="0">			
					...
					<tbody class="goods">	
					<?php if(is_array($goods_message)): $i = 0; $__LIST__ = $goods_message;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods): $mod = ($i % 2 );++$i;?><tr class="first last">
							<td class="first" style="width:40px;">
								</td> 
							<td class="content"> 
								<a class="content-link" target="_blank" href="<?php echo U('detail/index',array('gid'=>$goods['gid']));?>"> 
									<img src="/shop/Public/<?php echo ($goods["big_picture"]); ?>"/> 
									<b><span><?php echo ($goods["first_name"]); ?></span></b>
									<br><br><br>
									<span class="props">   
									<span>颜色：<?php echo ($goods["color"]); ?></span>    
									<span>尺码：<?php echo ($goods["size"]); ?></span>   
									</span>  
								</a> 
							</td> 
							<td> <div class="u_price"><?php echo ($goods["price"]); ?></div>   </td> 
							<td><?php echo ($goods["nums"]); ?></td> 
							<td><?php echo ($goods["price_all"]); ?></td> 
							<td class="last"></td> 
						</tr><?php endforeach; endif; else: echo "" ;endif; ?>
					<input type="hidden" name="nums" value="<?php echo ($i); ?>">
						<tr class="total">					
							<td class="first"></td>					
							...
							<td colspan="5" class="last">						
								<span class="right">														
									...
									<a href="javascript:void(0)" class="go_charge" id="charge">
										...
										<?php if(is_array($goods_message)): $z = 0; $__LIST__ = $goods_message;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods): $mod = ($z % 2 );++$z;?><input type="hidden" name="cart_<?php echo ($z); ?>" value="<?php echo ($goods["cid"]); ?>"><?php endforeach; endif; else: echo "" ;endif; ?>
										<input type="hidden" name="count_price" value="<?php echo ($count_price); ?>">
